...
#network IO paths
path = home_dir+'data/cifar10/training/'
...
def _forward(batch: Batch, is_training: bool) -> jnp.ndarray:
    ...
    '''
    cnn = networks.ResNet(num_classes, channels=64) #less downsampling in initial block than ResNet18
    # cnn = hk.nets.ResNet18(num_classes, resnet_v2=False, bn_config={'decay_rate': 0.9})
...
def loss_fn(params: hk.Params, state: hk.State, batch: Batch) -> Tuple[jnp.ndarray, Tuple[jnp.ndarray, hk.State]]:
    ...
    loss = softmax_xent + decay * l2_loss
...
for step in range(num_train_steps):
    if step % 100 == 0:
        ...
        train_data.append([step, train_accuracy])
        test_data.append([step, test_accuracy])
